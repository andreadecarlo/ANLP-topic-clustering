...
### 4. BERTopic online (incremental) — large corpora

For very large corpora, use the **online** variant: it processes documents in chunks with `partial_fit` (IncrementalPCA + MiniBatchKMeans + OnlineCountVectorizer) so you avoid loading everything in memory at once. After all chunks, it optionally refines topic representations (update_topics + Llama labels).

```bash
uv run anlp bertopic-online --year-min 2010 --year-max 2020 --max-docs 100000 --save models/bertopic_lyrics_online
```

Options:

- `--save PATH` — where to save the model (default: `models/bertopic_lyrics_online`)
- `--no-refine` — skip `update_topics` and Llama labels (faster, c-TF-IDF names only)

Tune in `config.py`: `BERTOPIC_ONLINE_CHUNK_SIZE`, `BERTOPIC_ONLINE_N_CLUSTERS`, `BERTOPIC_ONLINE_DECAY`, `BERTOPIC_ONLINE_N_COMPONENTS`.

### 5. Similar songs for a given song
...
### 6. Most representative songs for a topic
...
- **BERTopic online**: `BERTOPIC_ONLINE_CHUNK_SIZE`, `BERTOPIC_ONLINE_N_CLUSTERS`, `BERTOPIC_ONLINE_DECAY`, `BERTOPIC_ONLINE_N_COMPONENTS`
...
- `src/anlp/cli.py` – CLI entrypoint (`anlp data|octis|bertopic|bertopic-online|visualize|similar|representative`)
...
from anlp.bertopic_pipeline import fit_bertopic_on_lyrics, fit_bertopic_on_lyrics_online, get_topic_labels, load_bertopic_model
...
# BERTopic (batch) or online (incremental) + retrieval
model, docs_df, topics, probs = fit_bertopic_on_lyrics(2010, 2020, 50_000)
# For large corpora: model, docs_df, topics, probs = fit_bertopic_on_lyrics_online(2010, 2020, 100_000)
